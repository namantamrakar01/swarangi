...
async function uploadSVGLogo() {
  try {
    console.log('🎨 Uploading Swarangi SVG logo to Cloudinary...')

    const logoPath = join(__dirname, 'src/assets/swarangi-logo.svg')

    const uploadResponse = await cloudinary.uploader.upload(logoPath, {
      resource_type: 'auto',
      public_id: 'swarangi-logo-svg',
      folder: 'swarangi',
    })

    const logoUrl = uploadResponse.secure_url

    console.log('✅ SVG Logo uploaded successfully!')
